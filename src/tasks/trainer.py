...
def train_and_fit(args):
    ...
    if args.use_pretrained_blanks == 1:
        ...
        checkpoint_path = os.path.join(args.temp_folder_path, "test_checkpoint_%d.pth.tar" % args.model_no)
        logger.info("Loading model pre-trained on blanks at {}".format(checkpoint_path))
    ...
    losses_per_epoch, accuracy_per_epoch, test_f1_per_epoch = load_results(args)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        if accuracy_per_epoch[-1] > best_pred:
            ...
            output_model_path = os.path.join(args.temp_folder_path, "task_test_model_best_%d.pth.tar" % args.model_no)
            torch.save({
                    'epoch': epoch + 1,\
        ...
        if (epoch % 1) == 0:
            output_model_path = os.path.join(args.temp_folder_path, "task_test_checkpoint_%d.pth.tar" % args.model_no)
    ...
    figure_path = os.path.join(args.temp_folder_path, "task_loss_vs_epoch_%d.png" % args.model_no)
    ...
    figure_path = os.path.join(args.temp_folder_path, "task_train_accuracy_vs_epoch_%d.png" % args.model_no)
    plt.savefig(figure_path)
    ...
    figure_path = os.path.join(args.temp_folder_path, "task_test_f1_vs_epoch_%d.png" % args.model_no)
    plt.savefig(figure_path)
